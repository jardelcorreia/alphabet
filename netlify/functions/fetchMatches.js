...
const normalizeMatchStatus = (match) => {
  ...
  switch (upperStatus) {
    ...
    case 'TIMED':
      ...
      if (matchDate < now && (now - matchDate) > (3 * 60 * 60 * 1000)) {
        console.warn(`[STATUS WARNING] Jogo ${match.id} agendado mas já passou 3h da data: ${matchDate.toISOString()}`);
      }
      return { ...match, status: 'SCHEDULED', statusSource: 'normalized' };
...
const processApiData = (data) => {
  ...
  const processedMatches = data.matches.map(match => normalizeMatchStatus(match));
...
exports.handler = async (event, context) => {
  ...
  console.log(`[FETCH START] Iniciando busca para ${cacheKey}, forceRefresh: ${forceRefresh}`);
